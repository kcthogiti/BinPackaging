...
class BinAssignment:
        ...
        for ix, row in data_norm.iterrows():
            ...
            for ix1, row1 in boxes_df.iterrows():
                if(vol < row1["AvailableCapacity"]) & (diagonal <= row1["diagonal"]) & (dim1 <= row1["Min"]) & (dim2 <= row1["Max"]):
                    ...
                    break     
            delayed(self.sortBoxes)(boxes_df, total_volume, np.max(data_norm[np.isnan(data_norm["AssignedCRT"])]["Area"]), np.max(data_norm[np.isnan(data_norm["AssignedCRT"])]["Max"]))  
    ...
    def generateResults(self,  orderData, availableBoxes, outputflName):
        ...
        ls = []
        for order in orderData["ORDER_NO"].unique():
            data_norm = delayed(self.AssignBoxes)(orderData[orderData["ORDER_NO"] == order], availableBoxes)
            ls.append(data_norm)    

        Result = pd.concat(compute(*ls,get=multiprocessing.get))
